...
namespace TestBase
{
    ...
    public static class TestServerBuilder
    {
        ...
        /// Use <see cref="TestServer.Host"/>.<see cref="ServiceProvider"/> to get instances of complex dependencies defined in your Startup class.
        /// </remarks>
        public static TestServer RunningServerUsingStartup<TStartup>(string environmentName = "Development",
        ...
        /// <summary>
        /// Build a running server, similar to WebHost.Build(), but using <see cref="TestServer"/>,
        /// using <typeparamref name="TStartup"/> as a startup class and <paramref name="webProjectPhysicalPathForContentRoot"/> as ContentRoot
        /// </summary>
        /// <typeparam name="TStartup"></typeparam>
        /// <param name="webProjectPhysicalPathForContentRoot">This parameter is optional (i.e. can be correctly guessed) if the 
        /// the AssemblyName of your <typeparamref name="TStartup"/> matches the physical directory of Web project under test.
        /// Which it will if you haven't deliberately changed it.</param>
        /// <param name="environmentName"></param>
        /// <param name="featureCollection"></param>
        /// <returns></returns>
        /// <remarks>
        /// Use <see cref="TestServer.Host"/>.<see cref="ServiceProvider"/> to get instances of complex dependencies defined in your Startup class.
        /// </remarks>
        public static TestServer RunningServerUsingStartupAndContentRoot<TStartup>(string webProjectPhysicalPathForContentRoot = null,
            string environmentName = "Development",
            FeatureCollection featureCollection = null)
        {
            webProjectPhysicalPathForContentRoot = webProjectPhysicalPathForContentRoot ??
                                                   InternalGuessWebProjectPathFromAssemblyName(typeof(TStartup).GetTypeInfo().Assembly);

            var webHostBuilder = new WebHostBuilder()
                .UseContentRoot(webProjectPhysicalPathForContentRoot).ConfigureServices(InitializeServices<TStartup>)
                .UseEnvironment(environmentName)
                .UseStartup(typeof(TStartup));
            return new TestServer(webHostBuilder, featureCollection ?? new FeatureCollection());
        }
        ...
        /// <returns></returns>
        public static string InternalGuessWebProjectPathFromAssemblyName(Assembly startupAssembly, string projectFilePattern = "*.*proj")
        {
