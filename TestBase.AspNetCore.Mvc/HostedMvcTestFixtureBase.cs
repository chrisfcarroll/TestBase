...
namespace TestBase
{
    ...
    ///         test server and make a request.
    ///     </para>
    ///     Usage
    ///     <para>
    ///         Given an AspNetCore MVC project with a <code>Startup</code> class, a <see cref="Controller" />, and routing, a
    ///         TestFixture which inherits
    ///         from <see cref="HostedMvcTestFixtureBase" /> can test Controller actions like this:
    ///     </para>
    ///     <code>
    ...
    ///     <strong>
    ///         See
    ///         https://github.com/chrisfcarroll/TestBase/blob/netstandard20/TestBase.Tests/AspNetCoreMVC/WhenTestingControllers.cs
    ///         for an example.
    ///     </strong>
    /// </remarks>
    public class HostedMvcTestFixtureBase : IDisposable
    {
        ...
        ///     The directory path of the Project under test, assumed to be the ContentRoot for the server.
        ///     If not specified, we attempt to guess by searching for a directory named typeof(TStartup).GetTypeInfo().Assembly
        ///     under the solution root.
        /// </param>
        /// <returns>and <see cref="HttpClient" /> which can make requests to the application.</returns>
        public HttpClient GivenClientForRunningServer<TStartup>(
            ...
            string contentRoot = null)
        {
            ...
            TestServer = TestServerBuilder.RunningServerUsingStartup<TStartup>(contentRoot);
        ...
        static string GetProjectPath(Assembly startupAssembly)
        {
            ...
            var pathToCurrentlyExecutingTest = AppDomain.CurrentDomain.BaseDirectory;
            ...
            throw new
            Exception($"Solution root could not be located using application root {pathToCurrentlyExecutingTest}.");
        }
        ...
        /// <returns></returns>
        protected static HttpRequestHeaders GivenRequestHeaders(
            HttpClient      httpClient,
            string          headerName,
            params string[] headerValues)
        {
            Inconclusive.IfPreconditionFails(headerValues, x => x != null && x.Length > 0);
