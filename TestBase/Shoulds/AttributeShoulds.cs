...
namespace TestBase
{
    ...
    {
        /// <summary>
        ///     Assert that <paramref name="@this" /> has a
        ...
        public static Type ShouldValidateProperty(this Type @this, string property, Type attribute)
        {
            @this.GetProperty(property)
                 ...
                 .Count()
                 .ShouldBeGreaterThan(0, $"Type {@this}.{property} has no Attribute of type {attribute}");
            return @this;
        }

        public static Expression<Func<TClass, TMember>> ShouldBeValidationAttributeFor<TClass, TMember>(
            this Type                         attribute,
        ...
        public static Type ShouldNotHaveAttribute<T>(this Type @this)
        {
            @this.GetTypeInfo()
                 .GetCustomAttributes(typeof(T), true)
                 .Count(a => a.GetType() == typeof(T))
                 .ShouldEqual(0, "Expected to not find attribute {0} on type {1}", typeof(T), @this);
            return @this;
        }

        public static PropertyInfo ShouldHaveAttributeSatisfying<T>(
            this   PropertyInfo @this,
            params Action<T>[]  assertions) where T : Attribute
        {
            ShouldHaveAttribute<T>(@this);

            var attribute = @this.GetCustomAttribute<T>();

            foreach (var assert in assertions) assert(attribute);
            return @this;
        }
