...
namespace TestBase
{
    public static class StreamShoulds
    {
        ...
        /// <summary>
        ///     Assert this <paramref name="@this" />, considered as a byte[], contains <paramref name="expected" /> as a
        ///     substring.
        /// </summary>
        /// <returns>@this</returns>
        public static Stream ShouldContain(
            this Stream     @this,
            Stream          expected,
            string          message = null,
            params object[] args)
        {
            Assert.That(@this,
                        ...
                        message ?? $"{nameof(StreamShoulds)}{nameof(ShouldContain)}",
                        args);
            return @this;
        }
            ...
            params object[] args)
        {
            Assert.That(@this,
                        actual => StreamContains(actual, expected),
                        message
                     ?? $"{nameof(StreamShoulds)}{nameof(ShouldContain)} {expected.Take(MaxExpectedBytesToShowInFailureOutput).Select(x => (char) x).ToArray()}",
                        args);
            return @this;
        }

        /// <summary>
        ///     Synonym for <seealso cref="ShouldHaveSameStreamContentAs{T,TE}" /> Assert this <paramref name="@this" /> and
        ///     <paramref name="expected" /> are byte-for-byte identical
        /// </summary>
        /// <returns>@this</returns>
        public static T ShouldEqualByStreamContent<T, TE>(
            this T          @this,
            TE              expected,
            string          message = null,
            params object[] args) where T : Stream where TE : Stream
        {
        ...
        /// <returns>@this</returns>
        public static T ShouldHaveSameStreamContentAs<T, TE>(
            this T          @this,
            TE              expected,
            string          message = null,
            params object[] args) where T : Stream where TE : Stream
        {
            ...
            var  bufLeft    = new byte[32];
            var  bufRight   = new byte[32];
            long l          = 0;
            var  lbytesread = 0;
            var  rbytesread = 0;
            while ((lbytesread = left.Read(bufLeft, 0, MaxExpectedBytesToShowInFailureOutput))   != 0
                && (rbytesread = right.Read(bufRight, 0, MaxExpectedBytesToShowInFailureOutput)) != 0)
            {
                var mismatches = Enumerable.Range(0, Math.Min(lbytesread, rbytesread))
                                           .Where(i => bufLeft[i] != bufRight[i])
                                           .ToArray();
        ...
        public static bool StreamContains(Stream actual, byte[] expectedContent)
        {
            ...
            {
                var bufLeft = new byte[expectedContent.Length];
                for (var i = 0; i < expectedContent.Length; i++)
                {
                    left.Position = i;
                    left.Read(bufLeft, 0, expectedContent.Length);
                    if (bufLeft.EqualsByValue(expectedContent)) return true;
                }
            }
