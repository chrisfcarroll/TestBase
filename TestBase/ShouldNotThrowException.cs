...
namespace TestBase
{
    ...
    public class ShouldNotThrowException : Exception
    {
        ...
        /// <summary>Creates a new <see cref="ShouldNotThrowException"/> with message taken from <paramref name="exception"/></summary>
        ...
        /// <summary>Creates a new <see cref="ShouldNotThrowException"/> with message constructed
        /// by Asserting that <paramref name="action"/> doesn't throw. with comment,args=<paramref name="comment"/>,<paramref name="commentArgs"/></summary>
        ...
        {
            return new ShouldNotThrowException(Assertion.New( action, a=> BoolWithString.False( "Threw:" + ExpressionToCode.ToCode(action)), comment, commentArgs));
        }
    }
}