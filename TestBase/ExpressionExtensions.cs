...
namespace TestBase
{
    ...
    public static class ExpressionHelper
    {
        public static Expression<Func<TFrom, TTo>> Chain<TFrom, TMiddle, TTo>(
            this Expression<Func<TFrom, TMiddle>> inner,
            Expression<Func<TMiddle, TTo>>        outer)
        {
            return Expression.Lambda<Func<TFrom, TTo>>(
                                                       new SwapVisitor(outer.Parameters[0], inner.Body)
                                                      .Visit(outer.Body),
                                                       inner.Parameters
                                                      );
        }

        public static Expression<Func<TFrom1, TFrom2, TTo>> Chain<TFrom1, TFrom2, TMiddle, TTo>(
            this Expression<Func<TFrom1, TFrom2, TMiddle>> inner,
            Expression<Func<TMiddle, TTo>>                 outer)
        {
