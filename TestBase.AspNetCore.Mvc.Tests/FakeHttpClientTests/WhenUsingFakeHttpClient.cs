...
namespace TestBase.AspNetCore.Mvc.Tests.FakeHttpClientTests
{
    [TestFixture]
    public class WhenUsingFakeHttpClient
    {
        ...
        {
            var cannedResponseThis = new HttpResponseMessage(HttpStatusCode.OK)
                                     {Content = new StringContent("I Expected This")};
            var cannedResponseThat = new HttpResponseMessage(HttpStatusCode.Accepted)
                                     {Headers = {{"That-Header", "Value"}}};

            var uut = new FakeHttpClient()
                     .Setup(x => x.RequestUri.PathAndQuery.StartsWith("/this"))
                     .Returns(cannedResponseThis)
                     .Setup(x => x.RequestUri.PathAndQuery.StartsWith("/that"))
                     .Returns(cannedResponseThat)
        ...
        {
            var cannedResponse = new HttpResponseMessage(HttpStatusCode.Accepted)
                                 {Content = new StringContent("I Expected this")};
        ...
        {
            var thisResponse = new HttpResponseMessage(HttpStatusCode.OK)
                               {Content = new StringContent("I Expected This")};
            var thatResponse = new HttpResponseMessage(HttpStatusCode.Accepted) {Headers = {{"That-Header", "Value"}}};

            var uut = new FakeHttpClient()
                     .Setup(x => x.RequestUri.PathAndQuery.StartsWith("/this") && x.Method == HttpMethod.Put)
                     .Returns(thisResponse)
                     .Setup(x => x.RequestUri.PathAndQuery.StartsWith("/that"))
                     .Returns(thatResponse)
                     .Setup(x => x.RequestUri.PathAndQuery.StartsWith("/forbidden"))
                     .Returns(new HttpResponseMessage(HttpStatusCode.Forbidden));
            ...
            uut.PutAsync("http://localhost/this",
            ...
            uut.GetAsync("http://other/")
               ...
               .GetResult()
               .StatusCode.ShouldBe(HttpStatusCode.InternalServerError,
                                    "Other should have fallen back to InternalServerError");
        }
        ...
        public void Should_UseOnNoMatch__GivenNoMatch()
        {
            ...
            HttpResponseMessage NoMatchesReturn(HttpRequestMessage rm)
            {
                return new HttpResponseMessage(HttpStatusCode.Ambiguous)
                       {Headers = {{"Custom", rm.RequestUri.ToString()}}};
            }
