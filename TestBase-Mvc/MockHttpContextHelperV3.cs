...
namespace TestBase
{
    public class HttpRequestOverridableWrapper : HttpRequestWrapper
    {
        public HttpRequestOverridableWrapper(HttpRequest httpRequest, string appVirtualDir)
        : base(httpRequest)
        {
            ApplicationPath = appVirtualDir;
        }
    ...
    public static class MockHttpContextHelper
    {
        public static T WithHttpContextAndRoutes<T>(
            ...
            HttpApplication         applicationInstance              = null) where T : Controller
        {
            var requestUrl1          = requestUrl          ?? @this.GetType().Name;
            var applicationInstance1 = applicationInstance ?? new HttpApplication();
            return @this.WithHttpContextAndRoutes(
                                                  MockHttpContextBase(
                                                                      FakeHttpContextCurrent(appVirtualPath,
                                                                                             requestUrl1,
                                                                                             query,
        ...
        public static HttpContextBase MockHttpContextBase(HttpContext httpContext, string appVirtualDir = "/")
        {
            ...
            context.Setup(ctx => ctx.Request)
                   .Returns(new HttpRequestOverridableWrapper(httpContext.Request, appVirtualDir));
            ...
            context.Setup(ctx => ctx.Session)
                   .Returns(new HttpSessionStateWrapper(httpContext.Items["AspSession"] as HttpSessionState));
        ...
        static HttpContext FakeHttpContextCurrent(
            string          appVirtualPath,
            string          requestUrl,
            string          query,
            ...
            HttpApplication applicationInstance)
        {
            var request = new HttpRequest("",
                                          new UriBuilder("http", "localhost", 80, appVirtualDir + requestUrl)
                                         .Uri.ToString(),
                                          query);
            ...
            httpContext.User =
            new WindowsPrincipal(WindowsIdentity.GetCurrent() ?? new WindowsIdentity("FakeWindowsIdentity"));
