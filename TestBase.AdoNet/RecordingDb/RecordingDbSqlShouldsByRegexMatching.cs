...
namespace TestBase.AdoNet.RecordingDb
{
    public static class RecordingDbSqlShouldsByRegexMatching
    {
        ...
        /// <summary>
        ///     Verifies that a command was invoked on <paramref name="recordingDbConnection" /> which satisfied
            ...
            object                     expectedWhereClauseValue = null)
        {
            ...
            var invocation = recordingDbConnection.ShouldHaveInvoked(
                                                                     i => i.CommandText.Matches(verbpattern,
                                                                                                sqlRegexOpts)
                                                                       && i.CommandText.Matches(frompattern,
                                                                                                sqlRegexOpts),
           ...
           .Value;
            foreach (var field in fieldList ?? new string[] { })
            {
                ...
                columnList.ShouldMatch(string.Format(@"({0}|{1}){2}", openBracket, comma, fieldOrQuotedField),
                                       sqlRegexOpts,
                                       "Expected to Select column {0} but didn't see it",
                                       field);
            }
        ...
        /// <summary>
        ///     Verifies that an Insert command was invoked on <paramref name="recordingDbConnection" /> on table
            ...
            IEnumerable<string>        exceptProperties = null)
        {
            ...
            typeof(T).GetDbRehydratablePropertyNames()
                     .Where(s => !exceptProperties.Contains(s, StringComparer.InvariantCultureIgnoreCase));
            ...
            object                     updateSource)
        {
            ...
            var cmd =
            recordingDbConnection.ShouldHaveInvoked(c => c.CommandText.Matches(verbandtablepattern, sqlRegexOpts));
            ...
            foreach (var field in columnList)
            {
                ...
                if (updateSource != null)
                    FakeDbSqlShouldsByRegexMatching.ShouldHaveParameter(cmd,
                                                                        fieldName,
                                                                        updateSource
                                                                       .GetType()
                                                                       .GetProperty(fieldName)
                                                                       .GetValue(updateSource, null));
                else
                    cmd.Parameters
        ...
        /// <summary>
        ///     Verifies that a Delete command was invoked on <paramref name="recordingDbConnection" /> on table
        ///     <paramref name="tableName" />.
        /// </summary>
        /// <returns>The matching command</returns>
            ...
            object                     expectedWhereClauseValue)
        {
            ...
            var invocation = recordingDbConnection.ShouldHaveInvoked(
                                                                     ii => ii.CommandText.Matches(verbandtablepattern,
                                                                                                  sqlRegexOpts)
                                                                        && ii.Parameters.Cast<DbParameter>()
        ...
        /// <summary>
        ///     Verifies that an Update command was invoked on <paramref name="recordingDbConnection" /> on table
        ///     <paramref name="tableName" />.
        ...
        ///     <see cref="RecordingDbCommand.Invocations" />
        ///     Or by using <see cref="ShouldHaveUpdated{T}(RecordingDbConnection,string,T,string)" /> instead.
        /// </summary>
        /// <returns>The matching command</returns>
        public static DbCommand ShouldHaveUpdatedNRows<T>(
            this RecordingDbConnection recordingDbConnection,
            string                     tableName,
            T                          updateSource,
            int                        times = 1)
        {
            ...
            return ShouldHaveUpdatedNRows(recordingDbConnection,
                                          tableName,
            ...
            int                        times = 1)
        {
            ...
            var invocations =
            recordingDbConnection.Invocations.Where(i => i.CommandText.Matches(verbandtablepattern, sqlRegexOpts));
            ...
            foreach (var field in fieldList)
            {
                var field_             = field;
                var fieldOrQuotedField = string.Format(@"({0}|\[{0}\]|""{0}"")", field_);
                afterSet.ShouldMatch(string.Format(@"({0}|{1}){2}\s*=\s*\@{3}", set, comma, fieldOrQuotedField, field),
                                     sqlRegexOpts,
            ...
            string                     whereClauseProperty)
        {
            ...
            var expectedWhereValue = whereClausePropInfo != null
                                     ? whereClausePropInfo.GetPropertyValue(updateSource, whereClauseProperty)
                                     : null;

            return ShouldHaveUpdated(recordingDbConnection,
                                     tableName,
        ...
        ///     "Update tableName Set PropertyName1 = updateSource.PropertyName1 [, PropertyNameN = updateSource.PropertyNameN
        ///     ...]"
        ///     Verifies that the where clause includes "<paramref name="whereClauseProperty" /> =
            ...
            T                          whereClausePropertyExpectedValue)
        {
            ...
            ShouldHaveWhereClauseWithFieldEqualsExpected(invocation,
                                                         whereClauseProperty,
                                                         whereClausePropertyExpectedValue);
            return invocation;
        }

        public static DbCommand ShouldHaveWhereClauseWithFieldEqualsExpected<T>(
            this DbCommand invocation,
            string         columnName,
            T              expectedValue)
        {
            invocation.CommandText.ShouldMatch(@"Where "
                                             + optPrefix
                                             + optDelim
                                             + columnName
                                             + optDelim
            ...
            object         expectedValue)
        {
            Expression<Func<DbCommand, bool>> found = x => x.Parameters.Cast<DbParameter>()
                                                            .Any(
                                                                 p => p.ParameterName.Equals(parameterName,
                                                                                             StringComparison
            ...
            Assert.That(
                        ...
                        string.Format(
                                      ...
                                      parameterName,
                                      expectedValue,
                                      dbCommand.ToStringTextAndParams()));
            return dbCommand;
        }
    }
}
