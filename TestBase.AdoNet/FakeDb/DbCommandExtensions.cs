...
namespace TestBase.AdoNet
{
    public static class DbCommandExtensions
    {
        ...
        public static DataTable ToDataTable(this IEnumerable<object> fakeData, Type pocoTypeToReturn)
        {
            ...
            foreach (var property in propertyInfos) t.Columns.Add(new DataColumn(property.Name, property.PropertyType));

            foreach (var row in fakeData)
            {
                var items                                       = new object[propertyInfos.Length];
                for (var i = 0; i < items.Length; i++) items[i] = propertyInfos[i].GetValue(row, null);
                var dataRow =
                propertyInfos.Select(x => x.GetValue(row, null)).ToArray();
