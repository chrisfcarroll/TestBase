...
namespace TestBase.AdoNet
{
    public static class FakeDbSqlShouldsByRegexMatching
    {
        ...
        /// <summary>
        ///     Verifies that a command was invoked on <paramref name="fakeDbConnection" /> which satisfied
        ...
        ///     Optionally verifies that the where clause included "<paramref name="whereClauseField" /> =
        ///     <paramref name="expectedWhereClauseValue" />"
        /// </summary>
        /// <returns>The matching command</returns>
        public static DbCommand ShouldHaveSelected(
            this FakeDbConnection fakeDbConnection,
            string                tableName,
            ...
            object                expectedWhereClauseValue = null)
        {
            ...
            var invocation = fakeDbConnection.ShouldHaveInvoked(
                                                                ...
                                                                "Expected to Select from table {0} but didn't see it.",
                                                                tableName);

            var columnList =
            new Regex(@"(?<=" + select + ")" + restofline + @"(?=\s+From)", SqlRegexOpts)
           .Matches(invocation.CommandText)[0]
           .Value;
            foreach (var columnName in fieldList ?? new string[] { })
            {
                ...
                columnList.ShouldMatch(
                                       ...
                                       SqlRegexOpts,
                                       $"Expected to Select column {columnName} but didn't see it");
            }
        ...
        ///     Verifies that a Select command was invoked on <paramref name="fakeDbConnection" /> which
        ///     selected from table <paramref name="tableName" />.
        ///     Verifies that the select clause includes columns named for the public Properties of
        ///     <paramref name="withColumnsForEachRehydratablePropertyOf" /> which are
        ///     <seealso cref="DbRehydrationExtensions.GetDbRehydratablePropertyNames" />
        ///     Optionally verifies that the where clause included "<paramref name="whereClauseField" /> =
        ///     <paramref name="expectedWhereClauseValue" />"
        /// </summary>
        /// <returns>The matching command</returns>
        public static DbCommand ShouldHaveSelected<T>(
            this FakeDbConnection fakeDbConnection,
            string                tableName,
            ...
            object                expectedWhereClauseValue = null)
        {
            ...
            return ShouldHaveSelected(fakeDbConnection,
                                      tableName,
                                      fieldList,
                                      whereClauseField,
                                      expectedWhereClauseValue);
        }
        ...
        /// <summary>
        ///     Verifies that an Insert command was invoked on <paramref name="fakeDbConnection" /> on table
        ///     <paramref name="tableName" />.
        ///     Verifies that the column names, parameter names, and parameter values matche the Property names and values of
        ///     <paramref name="updateSource" />,
        ...
        ///     ...]"
        ///     And the Parameters are {Name="PropertyName1", Value=updateSource.PropertyName1} [, {Name="PropertyNameN",
        ///     Value=updateSource.PropertyNameN} ...]
        /// </summary>
        /// <returns>The matching command</returns>
        public static DbCommand ShouldHaveInserted<T>(
            this FakeDbConnection fakeDbConnection,
            string                tableName,
            T                     updateSource,
            IEnumerable<string>   exceptProperties = null)
        {
            ...
            typeof(T).GetDbRehydratablePropertyNames()
                     .Where(s => !exceptProperties.Contains(s, StringComparer.CurrentCultureIgnoreCase));
        ...
        /// </summary>
        /// <returns>The matching command</returns>
        public static DbCommand ShouldHaveInsertedColumns(
            this FakeDbConnection fakeDbConnection,
            string                tableName,
        ...
        ///     and returns the first such command recorded.
        ///     Verifies that the column names in <paramref name="columnList" /> were all updated
        ///     Optionally verifies that the parameter names and values matche the Property names and values of
        ///     <paramref name="updateSource" />,
        ///     i.e. that the command is something like
        ///     "Insert tableName Set PropertyName1 = updateSource.PropertyName1 [, PropertyNameN = updateSource.PropertyNameN
        ///     ...]"
        ...
        /// <returns>The matching command</returns>
        public static DbCommand ShouldHaveInserted(
            this FakeDbConnection fakeDbConnection,
            string                tableName,
            IEnumerable<string>   columnList,
            object                updateSource)
        {
            ...
            var cmd = fakeDbConnection.ShouldHaveInvoked(c => c.CommandText.Matches(verbandtablepattern, SqlRegexOpts),
                                                         "Expected to Insert table {0} but didn't see it",
                                                         tableName);
            ...
            var valuesList =
            new Regex(closeBracket + @"Values" + openBracket + restofline, SqlRegexOpts).Matches(cmd.CommandText)[0]
                                                                                        .Value;
            foreach (var field in columnList)
            {
                ...
                foundColumnList.ShouldMatch(string.Format(@"({0}|{1}){2}", openBracket, comma, fieldOrQuotedField),
                                            SqlRegexOpts,
                                            "Expected to Insert column {0} but didn't see it",
                                            field);
                valuesList.ShouldMatch(string.Format(@"(\(\s*|,\s*)@{0}\s*", field),
                                       SqlRegexOpts,
                                       "Expected to Insert value {0} but didn't see it",
                                       field);
                if (updateSource != null)
                    cmd.ShouldHaveParameter(fieldName,
                                            updateSource.GetType().GetProperty(fieldName).GetValue(updateSource, null));
                else
                    cmd.Parameters
                       .Cast<FakeDbParameter>()
                       .SingleOrAssertFail(p => p.ParameterName.Equals(fieldName,
        ...
        /// <summary>
        ///     Verifies that a Delete command was invoked on <paramref name="fakeDbConnection" /> on table
        ///     <paramref name="tableName" />.
        /// </summary>
        /// <returns>The matching command</returns>
        ...
        ///     Verifies that a Delete command was invoked on <paramref name="fakeDbConnection" /> on table
        ///     <paramref name="tableName" />.
        ///     Verifies that the where clause of the Delete command includes <paramref name="whereClauseFieldName" /> =
        ///     <paramref name="expectedWhereClauseValue" />
        /// </summary>
        /// <returns>The matching command</returns>
        public static DbCommand ShouldHaveDeleted(
            this FakeDbConnection fakeDbConnection,
            string                tableName,
            string                whereClauseFieldName,
            object                expectedWhereClauseValue)
        {
            var verbandtablepattern = @"Delete\s+" + optFrom + optPrefix + optDelim + tableName;

            var invocation = fakeDbConnection.ShouldHaveInvoked(
                                                                ii => ii.CommandText.Matches(verbandtablepattern,
                                                                                             SqlRegexOpts)
                                                                   && ii.Parameters.Cast<FakeDbParameter>()
                                                                        .Any(p => p.ParameterName
                                                                               == whereClauseFieldName
                                                                               && p
                                                                                 .Value
                                                                                 .Equals(expectedWhereClauseValue)),
                                                                "Expected to Delete {0} but found no matching Delete command.",
                                                                tableName);
        ...
        /// <summary>
        ///     Verifies that an Update command was invoked on <paramref name="fakeDbConnection" /> on table
        ///     <paramref name="tableName" />.
        ///     Verifies that the column names and parameter names match the Property names and values of
        ///     <paramref name="updateSource" />,
        ///     i.e. that the command is something like
        ...
        ///     You can do that with the <see cref="DbCommand.DbParameterCollection" /> property of the returned
        ///     <see cref="FakeDbCommand.Invocations" />
        ///     Or by using <see cref="ShouldHaveUpdated{T}(TestBase.AdoNet.FakeDbConnection,string,T,string)" /> instead.
        /// </summary>
        /// <returns>The matching command</returns>
        public static FakeDbCommand ShouldHaveExecutedNTimes<T>(
            this FakeDbConnection fakeDbConnection,
            string                verb,
            string                tableName,
            T                     updateSource,
            int                   times = 1)
        {
            ...
            updateSource.GetType()
                        .GetDbRehydratablePropertyNames()
                        .Where(s => !s.EndsWith("id", StringComparison.CurrentCultureIgnoreCase));

            return ShouldHaveExecutedNTimes(fakeDbConnection,
                                            verb,
                                            tableName,
                                            dbRehydratablePropertyNamesExIdFields,
                                            times);
        }
        ...
        /// </summary>
        /// <returns>The matching command</returns>
        public static FakeDbCommand ShouldHaveExecutedNTimes(
            this FakeDbConnection fakeDbConnection,
            string                verb,
            string                tableName,
            ...
            int                   times = 1)
        {
            ...
            var possiblyRelevantCommands =
            fakeDbConnection.Invocations.Where(c => c.CommandText.Matches(verbandtablepattern + optDelim,
                                                                          SqlRegexOpts));
            ...
            foreach (var cmd in possiblyRelevantCommands)
                try
                {
                    var afterVerbAndTable =
                    new Regex(@"(?<=" + verbandtablepattern + ")" + restofline, SqlRegexOpts)
                   .Matches(cmd.CommandText)[0]
                   .Value;
                    foreach (var field in fieldList)
                    {
                        ...
                        afterVerbAndTable.ShouldMatch(
                                                      string.Format(@"({0}|{1}){2}\s*=\s*\@{3}",
                                                                    set,
                                                                    comma,
                                                                    fieldOrQuotedField,
                                                                    field),
                                                      SqlRegexOpts,
                                                      "Expected to {0} field {1} but didn't see it",
                                                      verb,
                                                      field);
            ...
            matches.ShouldBe(
                             times,
                             string.Format("Expected to invoke {0} {1} commands against table {2} but got {3}",
                                           times,
                                           verb,
                                           tableName,
                                           matches));
        ...
        public static DbCommand ShouldHaveExecutedNTimes(this DbCommand dbCommand, int times = 1)
        {
            if (times != 1)
            {
                ...
                if (fakeDbCommand == null)
                    throw new
                    Assertion($"ShouldHaveUpdatedNRows(times != 1) can only be asserted against a {typeof(FakeDbCommand)} not a {dbCommand.GetType()}");
                else
        ...
        ///     Verifies that an Update command was invoked on <paramref name="fakeDbConnection" /> on table
        ///     <paramref name="tableName" />.
        ///     Verifies that the column names, parameter names and property values match the Property names and values of
        ///     <paramref name="updateSource" />,
        ///     i.e. that the command is something like
            ...
            string                whereClauseColumnName)
        {
            ...
            return ShouldHaveUpdated(fakeDbConnection,
                                     tableName,
                                     updateSource,
                                     whereClauseColumnName,
                                     expectedWhereValue);
        }
        ...
        /// </summary>
        /// <returns>The matching command</returns>
        public static DbCommand ShouldHaveUpdated<TSource, TWhere>(
            this FakeDbConnection fakeDbConnection,
            string                tableName,
            TSource               updateSource,
            string                whereClauseColumnName,
            TWhere                whereClauseColumnExpectedValue)
        {
            var dbParameterisablePropertyNamesExWhereClause = updateSource
                                                             .GetType()
                                                             .GetDbRehydratablePropertyNames()
                                                             .Where(s => s != whereClauseColumnName);
            return ShouldHaveUpdated(fakeDbConnection,
                                     tableName,
                                     dbParameterisablePropertyNamesExWhereClause,
                                     whereClauseColumnName,
                                     whereClauseColumnExpectedValue);
        }
        ...
        ///     "Update tableName Set PropertyName1 = updateSource.PropertyName1 [, PropertyNameN = updateSource.PropertyNameN
        ///     ...]"
        ///     Verifies that the where clause includes "<paramref name="whereClauseColumnName" /> =
        ///     <paramref name="whereClausePropertyExpectedValue" />"
        /// </summary>
        /// <returns>The matching command</returns>
        public static DbCommand ShouldHaveUpdated<T>(
            this FakeDbConnection fakeDbConnection,
            ...
            IEnumerable<string>   fieldList,
            string                whereClauseColumnName,
            T                     whereClausePropertyExpectedValue)
        {
                                                                ...
                                                                ii => ii.CommandText.Matches(verbandtablepattern,
                                                                   ...
                                                                   && ii.Parameters.Cast<FakeDbParameter>()
                                                                        .Any(p => p.ParameterName
                                                                               ...
                                                                               && p
                                                                                 .Value
                                                                                 .Equals(whereClausePropertyExpectedValue)),
                                                                "Expected to Update {0} but found no matching update command.",
                                                                tableName);
            ...
            ShouldHaveWhereClauseWithColumnEqualsExpected(invocation,
                                                          whereClauseColumnName,
                                                          whereClausePropertyExpectedValue);
            return invocation;
        }

        public static void ShouldHaveUpdatedFields(
            this DbCommand      invocation,
            IEnumerable<string> fieldList,
            bool                fromIdenticallyNamedParameters = false)
        {
            ...
            foreach (var field in fieldList)
            {
                ...
                SetClause.ShouldMatch(
                                      ...
                                      field);
                if (Regex.IsMatch(SetClause,
                                  string.Format(@"({0}|{1}){2}\s*=\s*\@{3}", set, comma, fieldOrQuotedField, field)))
                    invocation.Parameters.Cast<DbParameter>()
                              .SingleOrAssertFail(p => p.ParameterName.Equals(fieldName,
                                                                              StringComparison
        ...
        /// </summary>
        /// <returns>The matching command</returns>
        public static DbCommand ShouldHaveUpdatedFieldsFromIdenticallyNamedParameters<T>(
            this FakeDbConnection fakeDbConnection,
            string                tableName,
            IEnumerable<string>   fieldList,
            string                whereClauseColumnName,
        ...
        public static DbCommand ShouldHaveWhereClauseWithColumnEqualsExpected<T>(
            this DbCommand invocation,
            string         expectedWhereClauseColumnName,
            T              expectedValue)
        {
            invocation.CommandText.ShouldMatch(@"Where\s+.*"
                                             + optPrefix
                                             + optDelim
                                             + expectedWhereClauseColumnName
                                             + optDelim
                                             + @"\s*\=\s*@"
                                             + expectedWhereClauseColumnName,
                                               SqlRegexOpts,
                                               $"ShouldHaveWhereClauseWith {expectedWhereClauseColumnName} = @{expectedWhereClauseColumnName}");
            invocation.ShouldHaveParameter(expectedWhereClauseColumnName, expectedValue);
            return invocation;
        }

        public static DbCommand ShouldHaveWhereClauseMatching(
            this DbCommand invocation,
            string         expectedWhereClausePattern)
        {
            if (!Regex.IsMatch(expectedWhereClausePattern, @"^\s*Where\s+", SqlRegexOpts))
                expectedWhereClausePattern = @"Where\s+.*" + expectedWhereClausePattern;
            invocation.CommandText.ShouldMatch(expectedWhereClausePattern);
            return invocation;
        }

        public static DbCommand ShouldHaveWhereClauseContaining(
            this DbCommand invocation,
            string         expectedWhereClauseSubstring)
        {
            if (!Regex.IsMatch(expectedWhereClauseSubstring, @"^\s*Where\s+", SqlRegexOpts))
                expectedWhereClauseSubstring = @"Where\s+.*" + expectedWhereClauseSubstring;
        ...
        /// <summary>
        ///     Verifies that the <paramref name="dbCommand" />'s <see cref="DbCommand.DbParameterCollection" /> includes a
        ///     parameter
        ///     with the given <paramref name="parameterName" /> and <paramref name="expectedValue" />
        /// </summary>
        /// <returns>
        ///     <paramref name="dbCommand" />
        /// </returns>
        public static DbCommand ShouldHaveParameter(
